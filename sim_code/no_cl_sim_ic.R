...
for(i in 1:n_data){
  ...
  independant_no_cl[i, "elpd_loo"] <- loo_extract$elpd_loo
  independant_no_cl[i, "elpd_loo_se"] <- loo_extract$se_elpd_loo
  independant_no_cl[i, "p_loo"] <- loo_extract$p_loo
  independant_no_cl[i, "p_loo_se"] <- loo_extract$se_p_loo
  
  write.csv(independant_no_cl, "sim_results/independant_no_cl.csv", row.names = FALSE)
