...
parameters{
	...
	vector<lower=0, upper=pi()/2>[C] eta_unif;
...
transformed parameters{
	...
	for(i in 1:C) {
		lambda[i] <- tan(lambda_unif[i]);  // lambda ~ cauchy(0,1)
		eta[i] <- tan(eta_unif[i]);  // eta ~ cauchy(0,1)
	}
	
	tau <- tan(tau_unif);  // tau ~ cauchy(0,1)
	
	cl <- z .* lambda .* (tau * eta); // cross loadings horseshoe+ prior (Matt trick)
