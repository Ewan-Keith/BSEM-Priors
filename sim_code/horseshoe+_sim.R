...
for(i in 1:n_data){
  ...
  # save error values
  sparse_hs_plus[i, 1:30] <- pop_dif
  temp_errors <- sparse_hs_plus[i,1:30]  %>% 
    as.numeric()
  
  sparse_hs_plus[i, "RMSE"] <- sqrt(mean(temp_errors^2))
  ...
  sparse_hs_plus[i, "elpd_loo"] <- loo_extract$elpd_loo
  sparse_hs_plus[i, "elpd_loo_se"] <- loo_extract$se_elpd_loo
  sparse_hs_plus[i, "p_loo"] <- loo_extract$p_loo
  sparse_hs_plus[i, "p_loo_se"] <- loo_extract$se_p_loo
  
  write.csv(sparse_hs_plus, "sim_results/sparse_hs_plus.csv", row.names = FALSE)
