...
for(i in 1:n_data){
  ...
  # fit model
  fit <- stan(file = 'models/sem_cl_hs_reparam.stan', data = stan_dat, 
  ...
  # save error values
  independant_hs[i, 1:30] <- estimated_cl
  temp_errors <- independant_hs[i,1:30]  %>% 
    as.numeric()
  
  independant_hs[i, "RMSE"] <- sqrt(mean(temp_errors^2))
  ...
  independant_hs[i, "elpd_loo"] <- loo_extract$elpd_loo
  independant_hs[i, "elpd_loo_se"] <- loo_extract$se_elpd_loo
  independant_hs[i, "p_loo"] <- loo_extract$p_loo
  independant_hs[i, "p_loo_se"] <- loo_extract$se_p_loo
  
  write.csv(independant_hs, "sim_results/independant_hs.csv", row.names = FALSE)
