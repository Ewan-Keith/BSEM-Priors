...
# //////////////////////////////////////////////////
#### specify non-sparse simsem model ####
# //////////////////////////////////////////////////
...
# cross loadings, 2 per lv
# lv1 - 8/14, lv2 - 4/13, lv3 - 3/9
loading[c(8, 14), 1] <- NA
loading[c(4, 13), 2] <- NA
loading[c(3, 9), 3] <- NA
...
loadingValues[c(8, 14), 1] <- 0.2 # small cross loading
loadingValues[c(4, 13), 2] <- 0.4 # medium cross loading
loadingValues[c(3, 9), 3] <- 0.6 # large cross loading
...
for(i in 1:n_data){
  ...
  temp_data <- generate(CFA.Model, n = 500)
  
  write.csv(temp_data, 
            file = paste0("data/non-sparse/ns_sem_data_", i, ".csv"),
            row.names=FALSE)
}
