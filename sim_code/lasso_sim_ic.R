...
for(i in 1:n_data){
  ...
  # save error values
  independant_lasso[i, 1:30] <- estimated_cl
  temp_errors <- independant_lasso[i,1:30]  %>% 
    as.numeric()
  
  independant_lasso[i, "RMSE"] <- sqrt(mean(temp_errors^2))
  ...
  independant_lasso[i, "elpd_loo"] <- loo_extract$elpd_loo
  independant_lasso[i, "elpd_loo_se"] <- loo_extract$se_elpd_loo
  independant_lasso[i, "p_loo"] <- loo_extract$p_loo
  independant_lasso[i, "p_loo_se"] <- loo_extract$se_p_loo
  
  write.csv(independant_lasso, "sim_results/independant_lasso.csv", row.names = FALSE)
