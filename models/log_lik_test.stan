data{
	...
	int min_C; // number of cross loadings
...
parameters{
	...
	vector[min_C] cl; // cross loadings
...
for (m in 1:D) {
	...
	for(i in 1:N){
		...
		mu[i,3] <- b[3] + lam[3]*FS[i,1] + cl[1]*FS[i,3];
		mu[i,4] <- b[4] + lam[4]*FS[i,1];
		mu[i,5] <- b[5] + lam[5]*FS[i,1];
		mu[i,6] <- b[6] + lam[6]*FS[i,2];
		mu[i,7] <- b[7] + lam[7]*FS[i,2];
		mu[i,8] <- b[8] + lam[8]*FS[i,2] + cl[2]*FS[i,1];
		mu[i,9] <- b[9] + lam[9]*FS[i,2];
		mu[i,10] <- b[10] + lam[10]*FS[i,2];
		mu[i,11] <- b[11] + lam[11]*FS[i,3];
		mu[i,12] <- b[12] + lam[12]*FS[i,3];
		mu[i,13] <- b[13] + lam[13]*FS[i,3] + cl[3]*FS[i,2];
		mu[i,14] <- b[14] + lam[14]*FS[i,3];
		mu[i,15] <- b[15] + lam[15]*FS[i,3];
	}
}
