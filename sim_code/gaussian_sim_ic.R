...
for(i in 1:n_data){
  ...
  # save error values
  independant_gaus[i, 1:30] <- estimated_cl
  temp_errors <- independant_gaus[i,1:30]  %>% 
    as.numeric()
  
  independant_gaus[i, "RMSE"] <- sqrt(mean(temp_errors^2))
  ...
  independant_gaus[i, "elpd_loo"] <- loo_extract$elpd_loo
  independant_gaus[i, "elpd_loo_se"] <- loo_extract$se_elpd_loo
  independant_gaus[i, "p_loo"] <- loo_extract$p_loo
  independant_gaus[i, "p_loo_se"] <- loo_extract$se_p_loo
  
  write.csv(independant_gaus, "sim_results/independant_gaus.csv", row.names = FALSE)
